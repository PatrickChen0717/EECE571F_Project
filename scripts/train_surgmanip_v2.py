...
dataset_dir_env = os.getenv("SURGMANIP_DIR")
if not dataset_dir_env:
    raise ValueError("SURGMANIP_DIR is not set.")
dataset_dir = Path(dataset_dir_env).expanduser().resolve() / "dataset"
...
def train_one_epoch(
    ...
    for batch in pbar:
        ...
        for _step in range(P):
            ...
            last6 = model.encoder.add_virtual_root(last_feat)[
                :, 0, ..., :2
            ]  # (B,M,6,2)
        ...
        last_obs6 = model.encoder.add_virtual_root(last_obs_feat)[
            ..., :2
        ]  # (B,1,M,6,2)
