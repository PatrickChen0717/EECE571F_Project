...
if Enable_WandB:
    job_name = "EECE571F_Project_training" + "dataset: surgpose" + "Transformer Benchmark"
    args = {
        "lr": lr,
        "hidden_size": 128,
...
for epoch in range(NUM_EPOCHS):
    ...
    if (epoch + 1) % SAVE_INTERVAL == 0:
        os.makedirs(f"models_transformers/model_weights_{wandb_timestamp}", exist_ok=True)
        torch.save(model.state_dict(), f"models_transformers/model_weights_{wandb_timestamp}/epoch{epoch}.pth")