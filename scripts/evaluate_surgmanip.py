...
def plot_sample_prediction(model, sample, device="cpu", instr_id=0, kp_id=0):
    ...
    # observed part
    plt.plot(pred_traj[:O, 0], pred_traj[:O, 1], label="Observed (GT init)")

    # predicted future
    plt.plot(pred_traj[O:, 0], pred_traj[O:, 1], label="Predicted Future")
...
def plot_batch_predictions(
    model, dataset, device="cpu", instr_id=0, kp_id=0, num_samples=8
):
    """
    ...
    for i in range(num_samples):
        ...
        # observed part
        ax.plot(pred_traj[:O, 0], pred_traj[:O, 1], color="blue")

        # predicted future
        ax.plot(pred_traj[O - 1 :, 0], pred_traj[O - 1 :, 1], color="red")
...
plot_batch_predictions(model, ds, device=device, instr_id=0, kp_id=2, num_samples=8)
